...
public class CartViewModel extends AndroidViewModel {
    ...
    public void makeOrder() {
        if (MainActivity.currentUser == null) {
            Toast.makeText(MainActivity.INSTANCE.getApplication(), "Войдите или зарегистрируйтесь, чтобы сделать заказ", Toast.LENGTH_SHORT).show();

            return;
        }

        if ((harmonicas.getValue() == null || harmonicas.getValue().isEmpty())
                && (bayans.getValue() == null || bayans.getValue().isEmpty())
                && (accordions.getValue() == null || accordions.getValue().isEmpty())) {

            Toast.makeText(MainActivity.INSTANCE.getApplication(), "В корзине ничего нет :(", Toast.LENGTH_SHORT).show();

            return;
        }

        MainActivity.INSTANCE.setFragment(MakeOrderFragment.class,
                R.id.cart_layout, null,
                MainActivity.INSTANCE.getResources().getString(R.string.make_order),
                CartFragment.class);
    }
